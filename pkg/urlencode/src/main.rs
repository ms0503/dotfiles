...
fn encode(src: String) -> String {
    ...
    for c in src.chars() {
        let c = c as u32;
        if c < 0x80 {
            if c < 0x24 || c == 0x3c || c == 0x3e || c == 0x60 || c == 0x7b || c == 0x7d || c == 0x7f {
    ...
    for c in src.chars() {
        ...
        if is_encoded {
            if num_ind == 1 {
                ...
                num_ind = 0;
                let num = u8::from_str_radix(&String::from_utf8(num_buf.clone()).expect("Error: unexpected token."), 16).expect("Error: unexpected token.");
                println!("num: {:?}", &num);
                ...
                if char_ind == char_len {
                    dst.push_str(&String::from_utf8(char_buf.clone()[0..char_len].to_vec()).expect("Error: invalid utf8 sequence."));
                    char_ind = 0;
                    char_len = 0;
                }
                is_encoded = false;
